    ...
    <div style={{ overflow: "hidden", position: "absolute", width: "100%", height: "100%" }}>
      ...
      <div id="information_panel">
        <div className="information_on_panel">
          <text>Subject name: {username}</text>
          <br />
          <text>Torture Subject Code: {sessionStorage.getItem("TSC")}</text>
          <br />
          <text>Simulations failed: {simulation_failed}</text>
          <br />
          <text>Simulations success: {simulation_success}</text>
          <br />
        </div>
        <div id="pulse" />
      </div>
      <button onClick={hide_stats} style={{ backgroundColor: "white", position: "absolute" }}>
        Go back
      </button>
    </div>
  );
